...
class DetalleCuentaCobroWindow:
    def __init__(self, company_code, billing_id, conn):
        ...
        tk.Label(self.root, text=f"Detalle Cuenta de Cobro #{billing_id}", font=("Arial", 16)).pack(pady=10)

        # Obtener información de la cuenta
        c = self.conn.cursor()
        c.execute("SELECT month_year, total_amount FROM billing_records WHERE id = ?", (billing_id,))
        billing = c.fetchone()
        if billing:
            tk.Label(self.root, text=f"Mes/Año: {billing['month_year']} | Total: ${billing['total_amount']:.2f}",
        ...
        # Cargar detalles
        detalles = self.cobro_manager.get_detalle_cuenta_cobro(company_code, billing_id)
